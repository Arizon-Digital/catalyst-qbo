...
console.log('@@@ DraftModeScript', { isDraft, appOrigin, searchParamName, headerName, originalUrl })

if (window.parent !== window) {
  window.addEventListener('message', event => {
    if (event.origin === appOrigin && event.data.type === 'makeswift_draft_mode') {
      console.log('@@@ DraftModeScript message', { event })
      ...
      if (!isDraft && !originalUrl.searchParams.has(searchParamName)) {
        ...
        console.log('@@@ DraftModeScript window.location.replace(url)', { url })
        ...
        window.fetch = function patchedFetch(resource, options) {
          ...
          const isServerAction = options?.method === 'POST' && options?.headers?.['Next-Action'] != null
          console.log('@@@ patchedFetch', { resource, isServerAction }, options)

          if (new URL(request.url).origin !== window.location.origin || isServerAction) {
