...
export default function BulkPricing({ product }: BulkPricingProps) {
  ...
  const renderBulkPricingDetails = () => {
    return product?.prices?.bulkPricing?.map((pricing, index) => {
      ...
      if ('price' in pricing) {
        discountText = `Fixed price: ${pricing.price}`;
      } else if ('percentOff' in pricing) {
        discountText = `${pricing.percentOff}% off`;
      } else if ('priceAdjustment' in pricing) {
        discountText = `${pricing.priceAdjustment}`;
      }

      return (
        <li key={index} className="text-gray-700">
          • Buy {pricing.minimumQuantity} - {pricing.maximumQuantity || 'above'} and get {discountText}
        </li>
      );
    });
  };
  ...
  if (!hasBulkPricing) {
    return (
      <div className="mt-4">
        ...
        <div className="ml-2 mt-[61px] productView-info-value">
          <Link 
            href="/bulk-pricing"
